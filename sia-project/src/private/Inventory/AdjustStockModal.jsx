...
const AdjustStockModal = ({ medicine, onClose, onSave }) => {
  ...
  const handleSave = () => {
    ...
    onSave({ ...medicine, stock: newStock });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96 text-gray-900 flex flex-col">
        <h2 className="text-lg font-bold mb-4 text-center">Adjust Stock</h2>
        ...
        <table className="w-full border-collapse border border-gray-300 mb-4 flex-grow">
          <tbody>
            ...
            <tr>
              ...
              <td><input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-full p-2 border text-gray-900 bg-white" /></td>
            </tr>
            <tr>
              <td className="border px-4 py-2">Reason (Optional)</td>
              <td><input type="text" value={reason} onChange={(e) => setReason(e.target.value)} className="w-full p-2 border text-gray-900 bg-white" /></td>
            </tr>
          </tbody>
        </table>
